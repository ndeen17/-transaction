...
import { ApiRequestError, listMyBankDeposits, type BankDepositSummary } from "../../lib/api";

const PAGE_SIZE = 20;

function statusBadgeClass(status: BankDepositSummary["status"]) {
  if (status === "credited") return "bg-[#F0FDF4] text-[#16A34A]";
  if (status === "rejected") return "bg-[#FEF2F2] text-[#DC2626]";
  return "bg-[#FFFBEB] text-[#B45309]";
}

function statusLabel(status: BankDepositSummary["status"]) {
...
export function BankDepositsListPage() {
  ...
  const [items, setItems] = useState<BankDepositSummary[]>([]);
  ...
  useEffect(() => {
    if (!token) return;
    setLoading(true);
    listMyBankDeposits(token, { page, limit: PAGE_SIZE })
      .then((result) => {
        setItems(result.items);
        setTotalPages(result.totalPages);
      })
      .catch((err) => setError(err instanceof ApiRequestError ? err.message : "Couldn't load bank deposits."))
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-28 lg:pb-0 lg:pl-64">
      ...
      <div className="mx-auto max-w-[720px] px-4 pb-10 pt-6 sm:px-6 sm:pt-10">
        <h1 className="text-xl font-semibold text-[#111827]">Bank deposits</h1>
        ...
        <div className={`${DASH_CARD} mt-5`}>
          ...
          {!loading && !error && items.length === 0 && (
            <p className="p-6 text-sm text-[#6B7280]">No bank deposits yet.</p>
          )}
            ...
            <div className="divide-y divide-[#E5E7EB]">
              {items.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => navigate(`/dashboard/bank-deposits/${item.id}`)}
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[#111827]">
                      {item.bankName} — {formatCurrency(item.amount, item.currency)}
                    </p>
