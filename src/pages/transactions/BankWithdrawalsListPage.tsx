...
import { ApiRequestError, listMyBankWithdrawals, type BankWithdrawalSummary } from "../../lib/api";

const PAGE_SIZE = 20;

function statusBadgeClass(status: BankWithdrawalSummary["status"]) {
  if (status === "completed") return "bg-[#F0FDF4] text-[#16A34A]";
  if (status === "declined") return "bg-[#FEF2F2] text-[#DC2626]";
  return "bg-[#FFFBEB] text-[#B45309]";
}

function statusLabel(status: BankWithdrawalSummary["status"]) {
  if (status === "completed") return "Completed";
  if (status === "declined") return "Declined";
  return "Processing";
}

export function BankWithdrawalsListPage() {
  const navigate = useNavigate();
  const { token, cachedUser } = useAuthedUser();
  const { toast, show } = useComingSoonToast();

  const [items, setItems] = useState<BankWithdrawalSummary[]>([]);
  ...
  useEffect(() => {
    if (!token) return;
    setLoading(true);
    listMyBankWithdrawals(token, { page, limit: PAGE_SIZE })
      .then((result) => {
        setItems(result.items);
        setTotalPages(result.totalPages);
      })
      .catch((err) => setError(err instanceof ApiRequestError ? err.message : "Couldn't load bank withdrawals."))
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-28 lg:pb-0 lg:pl-64">
      ...
      <div className="mx-auto max-w-[720px] px-4 pb-10 pt-6 sm:px-6 sm:pt-10">
        <h1 className="text-xl font-semibold text-[#111827]">Bank withdrawals</h1>
        ...
        <div className={`${DASH_CARD} mt-5`}>
          ...
          {!loading && !error && items.length === 0 && (
            <p className="p-6 text-sm text-[#6B7280]">No bank withdrawals yet.</p>
          )}
            ...
            <div className="divide-y divide-[#E5E7EB]">
              {items.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => navigate(`/dashboard/bank-withdrawals/${item.id}`)}
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[#111827]">
                      {item.bankName} withdrawal — {formatCurrency(item.amount, item.currency)}
                    </p>
