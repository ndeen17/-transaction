...
import { ApiRequestError, listMyCryptoDeposits, type CryptoDepositSummary } from "../../lib/api";
...
export function CryptoDepositsListPage() {
  const navigate = useNavigate();
  const { token, cachedUser } = useAuthedUser();
  const { toast, show } = useComingSoonToast();

  const [items, setItems] = useState<CryptoDepositSummary[]>([]);
  ...
  useEffect(() => {
    if (!token) return;
    setLoading(true);
    listMyCryptoDeposits(token, { page, limit: PAGE_SIZE })
      .then((result) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-28 lg:pb-0 lg:pl-64">
      ...
      <div className="mx-auto max-w-[720px] px-4 pb-10 pt-6 sm:px-6 sm:pt-10">
        <h1 className="text-xl font-semibold text-[#111827]">Crypto deposits</h1>
        ...
        <div className={`${DASH_CARD} mt-5`}>
          ...
          {!loading && !error && items.length === 0 && (
            <p className="p-6 text-sm text-[#6B7280]">No crypto deposits yet.</p>
          )}
            ...
            <div className="divide-y divide-[#E5E7EB]">
              {items.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => navigate(`/dashboard/crypto-deposits/${item.id}`)}
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-[#111827]">
                      {item.symbol} deposit — {formatCurrency(item.amount, item.currency)}
                    </p>
