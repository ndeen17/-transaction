...
import { ApiRequestError, adminListBankWithdrawals, type AdminBankWithdrawalSummary } from "../../lib/api";
...
export function VaultAdminBankWithdrawalsPage() {
  const token = localStorage.getItem("adminToken");

  const [items, setItems] = useState<AdminBankWithdrawalSummary[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    adminListBankWithdrawals(token, { status: statusFilter || undefined, limit: 50 })
      ...
      <div className="mx-auto max-w-[960px] px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className={`${DASH_CARD} mt-5`}>
          ...
          {!loading && !error && items.length === 0 && (
            <p className="p-6 text-sm text-[#6B7280]">No bank withdrawals match this filter.</p>
          )}
          {!loading && items.length > 0 && (
            <div className="divide-y divide-[#E5E7EB]">
              {items.map((item) => (
                <Link
                  key={item.id}
                  to={`/vaultadmin/bank-withdrawals/${item.id}`}
