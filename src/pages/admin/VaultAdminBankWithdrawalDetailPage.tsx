...
import {
  ...
  adminGetBankWithdrawal,
  type AdminBankWithdrawalSummary,
} from "../../lib/api";
...
export function VaultAdminBankWithdrawalDetailPage() {
  const { id } = useParams<{ id: string }>();
  const token = localStorage.getItem("adminToken");

  const [request, setRequest] = useState<AdminBankWithdrawalSummary | null>(null);
  ...
  useEffect(() => {
    if (!token || !id) return;
    adminGetBankWithdrawal(token, id)
      .then(setRequest)
  ...
  async function handleAccept() {
    ...
    setNotice(null);
    try {
      await adminAcceptBankWithdrawal(token, id);
      const fresh = await adminGetBankWithdrawal(token, id);
      setRequest(fresh);
  ...
  async function handleDecline() {
    ...
    setNotice(null);
    try {
      await adminDeclineBankWithdrawal(token, id, note || undefined);
      const fresh = await adminGetBankWithdrawal(token, id);
      setRequest(fresh);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-[640px] px-4 py-8 sm:px-6 sm:py-10">
        <Link to="/vaultadmin/bank-withdrawals" className="text-sm font-medium text-[#6B7280] hover:text-[#111827]">
          ← Bank withdrawals
        </Link>
            ...
            <div className="mt-6 flex items-center justify-between">
              <div>
                <h1 className="text-xl font-semibold text-[#111827]">
                  {request.bankName} withdrawal — {formatCurrency(request.amount, request.currency)}
                </h1>
