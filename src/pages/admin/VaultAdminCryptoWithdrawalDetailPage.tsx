...
import {
  ...
  adminGetCryptoWithdrawal,
  type AdminCryptoWithdrawalSummary,
} from "../../lib/api";
...
export function VaultAdminCryptoWithdrawalDetailPage() {
  const { id } = useParams<{ id: string }>();
  const token = localStorage.getItem("adminToken");

  const [request, setRequest] = useState<AdminCryptoWithdrawalSummary | null>(null);
  ...
  useEffect(() => {
    if (!token || !id) return;
    adminGetCryptoWithdrawal(token, id)
      .then(setRequest)
      .catch((err) => setLoadError(err instanceof ApiRequestError ? err.message : "Couldn't load this withdrawal."));
  ...
  async function handleAccept() {
    ...
    setNotice(null);
    try {
      await adminAcceptCryptoWithdrawal(token, id);
      const fresh = await adminGetCryptoWithdrawal(token, id);
      setRequest(fresh);
      setNotice("Approved — the customer has been notified.");
    } catch (err) {
  ...
  async function handleDecline() {
    ...
    setNotice(null);
    try {
      await adminDeclineCryptoWithdrawal(token, id, note || undefined);
      const fresh = await adminGetCryptoWithdrawal(token, id);
      setRequest(fresh);
      setNotice("Declined — the customer has been notified and refunded.");
    } catch (err) {
      setActionError(err instanceof ApiRequestError ? err.message : "Something went wrong.");
    } finally {
      setActing(false);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-[640px] px-4 py-8 sm:px-6 sm:py-10">
        <Link to="/vaultadmin/crypto-withdrawals" className="text-sm font-medium text-[#6B7280] hover:text-[#111827]">
          ← Crypto withdrawals
        </Link>
        ...
        {request && (
          <>
            <div className="mt-6 flex items-center justify-between">
              <div>
                <h1 className="text-xl font-semibold text-[#111827]">
                  {request.symbol} withdrawal — {formatCurrency(request.amount, request.currency)}
                </h1>
            ...
            <div className={`${DASH_CARD} mt-5 p-5 sm:p-6`}>
              <h2 className="text-[15px] font-semibold text-[#111827]">Withdrawal details</h2>
              <dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
                <Field label="Reference" value={request.reference} />
                <Field label="Amount" value={`${request.amountCrypto} ${request.symbol}`} />
                <Field label="USD value" value={formatCurrency(request.amount, request.currency)} />
                <Field label="Wallet address" value={request.walletAddress} />
              ...
              {request.status === "declined" && request.adminNote && (
                <p className="mt-4 rounded-xl bg-[#FEF2F2] px-4 py-3 text-sm text-[#DC2626]">
                  Decline note: {request.adminNote}
                </p>
              )}
              ...
              {request.status === "processing" && (
                <div className="mt-5 space-y-4">
                  <DashField label="Decline note" optional>
                    <input
                      value={note}
                      onChange={(e) => setNote(e.target.value)}
                      placeholder="Shown to the customer if you decline"
                  ...
                  <div className="flex gap-2">
                    ...
                    <DashboardButton variant="danger" onClick={handleDecline} disabled={acting}>
                      {acting ? "Working…" : "Decline & refund"}
                    </DashboardButton>
                  </div>
                </div>
              )}
